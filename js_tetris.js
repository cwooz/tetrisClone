...
function detectCollision() {
	...
	if (
		...
		(currTetro.hit1.data[0] != 0 ||
			currTetro.hit2.data[1] != 0 ||
			currTetro.hit3.data[0] != 0)
	) {
		rowCheck();
		resetTetro();
		copyCanvas();
...
function getHitInfo() {
	currTetro.hit1 = ctx.getImageData(
		tetro.position.x + currTetro.boundaryPoints[0],
		tetro.position.y + currTetro.boundaryPoints[1] + 1,
		1,
		1
	);
	currTetro.hit2 = ctx.getImageData(
		tetro.position.x + currTetro.boundaryPoints[2],
		tetro.position.y + currTetro.boundaryPoints[3] + 1,
		1,
		1
	);
	currTetro.hit3 = ctx.getImageData(
		tetro.position.x + currTetro.boundaryPoints[4],
		tetro.position.y + currTetro.boundaryPoints[5] + 1,
		1,
		1
	);
}
...
function moveX(eraseArray) {
	...
	tetro.position.x = Math.max(
		0,
		Math.min(tetro.position.x, W - currTetro.image.width)
	);
